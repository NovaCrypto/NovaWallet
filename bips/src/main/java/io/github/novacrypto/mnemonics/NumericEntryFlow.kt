    ...
    fun modelStream(): Observable<NumericEntryModel> =
            input.scan(initialState(),
                    { model: NumericEntryModel, event: NumericEntryEvent ->
                        when (event) {
                            is NumericEntryNumberEvent -> onNumberPress(model, event, root)
                            is NumericEntryBackspaceEvent -> onBackspacePress(model)
                            is NumericEntryAcceptEvent -> onAcceptPress(model, event, root)
                        }
                    })
...
private fun onAcceptPress(model: NumericEntryModel, event: NumericEntryAcceptEvent, root: NumericTree): NumericEntryModel {
    return model.copy(
            currentKey = "",
...
private fun onNumberPress(model: NumericEntryModel, event: NumericEntryNumberEvent, root: NumericTree): NumericEntryModel {
